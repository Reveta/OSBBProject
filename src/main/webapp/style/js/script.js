...
function getTopDate() {
  ...
  var topDate = new Date(),
      code = '<span class="date-day">' + topDate.getDate() + '</span>' +
          '<span class="date-meta">' + '<span class="date-month">'
          + topDate.getMonthName() + '</span>' + '<br>' +
          '<span class="date-year">' + topDate.getFullYear() + '</span>'
          + '</span>';
...
function carouselStart() {
  ...
  function prevCarousel(carousel, itemWidth, speed) {
    $(carousel).find(".carousel-items .carousel-item").eq(-1).clone().prependTo(
        $(carousel).find(".carousel-items"));
  ...
  function nextCarousel(carousel, itemWidth, speed) {
    $(carousel).find(".carousel-items .carousel-item").eq(0).clone().appendTo(
        $(carousel).find(".carousel-items"));
    $(carousel).find(".carousel-items").css({"left": "0px"});
    $(carousel).find(".carousel-items .carousel-item").eq(0).remove();
    $(carousel).find(".carousel-items").animate({left: "-" + itemWidth + "px"},
        speed);
  }

  $(".carousel-button").on("click", function () {
    var id = $(this).attr("id"), speed = 800,
        carousel = $(this).parents(".carousel"),
        itemWidth = $(carousel).find(".carousel-item").outerWidth();
...
function workWithNews() {
  ...
  var newsName = newsName, newsShort = newsShort, newsText = newsText,
      newsTime = newsTime, newsAuthor = newsAuthor,
      newsImage = '../style/images/news' + newsId + '.jpg';

  $(".news-slider-item").css(
      {"background-image": "url(" + newsImage + ")"}).removeAttr("id").attr(
      "id", "news-slider-item-" + newsId);
